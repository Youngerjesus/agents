---
description: Create a comprehensive technical design document including architecture, component interactions, and detailed logic flows.
handoffs:
  - label: Review Design
    agent: architect
    prompt: Review the generated design for scalability and maintainability.
---
...
1.  **Context Analysis**:
    - Identify the target feature or system scope from `$ARGUMENTS` or the currently open file.
    - Read the existing Spec (`spec.md`) and Plan (`plan.md`) if available to understand the requirements and data contracts.
    - If no specific files are found, assume the scope is the user input itself.

2.  **Design Execution**:
    Create or update a design document (default: `docs/design/[feature-name]-design.md` or simply `docs/designs/[feature-name].md`) with the following sections. **Focus on *HOW* it works, not *WHAT* it does.**
...
3.  **Review Criteria**:
    - Ensure strict separation of concerns (Layered Architecture).
    - Design for testability (can we mock dependencies?).
    - Security implications (authz/authn checks).

4.  **Output**:
    - Confirm the design file path.
    - Write the content to the design file.
    - Report completion with a brief summary of the design decisions made.
